					...
					function() {
						...
						if (input.attr("name") != 'st_layout_boxed') {
							...
							if (!input.prop("checked")) {
								theme_settings[input.attr("name")] = input
										.val();
							} else {
						...
						if (input.attr("name") === 'st_sb_scroll') {
							...
							} else if (theme_settings.st_sb_scroll == 1
									&& theme_settings.st_layout_boxed == 1) {
								theme_settings.st_sb_fixed = -1;
							} else if (theme_settings.st_sb_scroll == 0
									&& theme_settings.st_layout_boxed == 1) {
								theme_settings.st_sb_fixed = -1;
							} else {
...
function set_settings(theme_settings, option) {
	...
	if (theme_settings.st_sb_scroll == 1) {
		...
		$(".page-sidebar").removeClass("scroll").css("height", "")
	...
	if (option === false || option === 'st_layout_boxed'
			|| option === 'st_sb_fixed' || option === 'st_sb_scroll') {
...
function set_settings_checkbox(name, value) {
	...
	if (name == 'st_layout_boxed') {
		...
		$(".theme-settings").find("input[name=" + name + "]").prop("checked",
				false).parent("div").removeClass("checked");
		...
		input.parent("div").removeClass("disabled").parent(".check")
		...
		if (value === -1) {
			...
			input.parent("div").addClass("disabled").parent(".check").addClass(
					"disabled");
		}
