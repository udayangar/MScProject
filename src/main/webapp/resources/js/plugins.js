$(function() {
	...
	var feMasked = function() {
		if ($("input[class^='mask_']").length > 0) {
			$("input.mask_sample").mask('99-9999999');
			$("input.mask_date").mask('9999-99-99');
			$("input.mask_contract").mask('a*-999-a999');
			$("input.mask_phone").mask('9999-999 999');
			$("input.mask_international_phone").mask('(+99) 999-999 999');
			$("input.mask_phone_ext").mask('99 (999) 999-9999? x99999');
			$("input.mask_credit").mask('9999-9999-9999-9999');
			$("input.mask_hour").mask('99:99:99');
			$("input.mask_percent").mask('99%');
			$("input.mask_ip").mask('9999.9999.9999.9999');
		}
	}
	// END Masked Inputs
			...
			if ($(".summernote_lite").length > 0) {
				...
				$(".summernote_lite").on("focus", function() {
					...
					$(".summernote_lite").summernote({
						height : 100,
						focus : true,
						toolbar : [ [ "style", [ "bold", "italic", "underline", "clear" ] ], [ "insert", [ "link", "picture", "video" ] ] ]
					});
				});
			}
				...
				$(".summernote_email").summernote(
						{
							height : 400,
							focus : true,
							toolbar : [ [ 'style', [ 'bold', 'italic', 'underline', 'clear' ] ], [ 'font', [ 'strikethrough' ] ],
		...
		if (action == 'show' || action == 'update') {
			...
			if (action == 'show') {
				...
				var mpb = '<div class="mpb ' + settings.position + '">\n\
						...
						+ (settings.state != '' ? ' mpb-' + settings.state : '') + '" style="width:' + settings.value[0]
						+ '%;"></div>\n\
                           </div>';
				$('body').append(mpb);
			}
